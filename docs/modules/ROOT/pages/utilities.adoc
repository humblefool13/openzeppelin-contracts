...
The xref:api:utils.adoc#Strings[`Strings`] library provides helpers for formatting and parsing values as strings, along with utilities for basic ASCII case conversion.
...
NOTE: `toLower` and `toUpper` operate on ASCII only: bytes in `A`-`Z` and `a`-`z` are converted; other bytes are left unchanged. These are not Unicode-aware.
